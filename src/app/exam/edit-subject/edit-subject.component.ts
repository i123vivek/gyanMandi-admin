...
export class EditSubjectComponent implements OnInit, OnDestroy {
  ...
  get subjectArrayFun() {
    return this.editSubjectForm.get('subjectArray') as FormArray;
  }
  ...
  selectphase_count(event: any) {
    ...
    for (let x of this.examList) {
      if (x.examId == this.examId) {
        this.editSubjectForm.controls['examName'].setValue(x.examName);
        this.editSubjectForm.controls['phase_count'].setValue(x.phase_count);
      }
    }
    this.phase_count = this.editSubjectForm.controls['phase_count'].value
    //console.log("phase_count is", this.editSubjectForm.controls['phase_count'].value)
  ...
  public addSubject = () => {
    ...
    formData.append('is_active',this.editSubjectForm.controls['subjectStatus'].value)
